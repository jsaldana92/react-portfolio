...
export default function SEEHBtimeline() {
  ...
  useLayoutEffect(() => {
    ...
    const waitForImages = () => {
      const imgs = [];
      if (firstRef.current)
        imgs.push(...firstRef.current.querySelectorAll("img"));
      if (secondRef.current)
        imgs.push(...secondRef.current.querySelectorAll("img"));
      if (finalSectionRef.current && finalRef.current)
        imgs.push(finalRef.current);
